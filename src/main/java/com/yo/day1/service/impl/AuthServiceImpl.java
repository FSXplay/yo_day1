...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse refresh(RefreshTokenRequest request) {
        ...
        Instant accessExpiresAt = now.plusSeconds(appJwtProperties.accessTokenTtlMinutes() * 60);
        Instant nextRefreshExpiresAt = now.plusSeconds(appJwtProperties.refreshTokenTtlDays() * 24 * 60 * 60);
    ...
    private AuthResponse buildTokensForUser(User user, String rawPassword) {
        ...
        Instant now = Instant.now();
        Instant accessExpiresAt = now.plusSeconds(appJwtProperties.accessTokenTtlMinutes() * 60);
        Instant refreshExpiresAt = now.plusSeconds(appJwtProperties.refreshTokenTtlDays() * 24 * 60 * 60);
